...
from onsets_and_frames.transcriber import OnsetsAndFrames_TCN, OnsetsAndFrames_biTCNv2
...
def config():
    ...
    logdir = 'runs/BiTCNv2-' + ','.join(str(x) for x in TCN_layers) + '-' + datetime.now().strftime('%y%m%d-%H%M%S')
...
def train(logdir, device, iterations, resume_iteration, checkpoint_interval, train_on, batch_size, sequence_length,
    ...
    if resume_iteration is None:
        model = OnsetsAndFrames_biTCNv2(N_MELS, MAX_MIDI - MIN_MIDI + 1, TCN_layers, 2, model_complexity).to(device)
