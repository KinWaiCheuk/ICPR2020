...
def config():
    ...
    logdir = f'runs_AE/WNet-z2_only-semi_training_from0-same_size-'+ datetime.now().strftime('%y%m%d-%H%M%S')
...
def train(logdir, device, iterations, resume_iteration, checkpoint_interval, train_on, batch_size, sequence_length,
    ...
    if train_on == 'MAESTRO':
        ...
        semi_dataset = dataset = MAESTRO(groups=train_groups, sequence_length=sequence_length)
        validation_dataset = MAPS(groups=['ENSTDkAm', 'ENSTDkCl'], sequence_length=validation_length, refresh=refresh)
    ...
    semi_loader = DataLoader(semi_dataset, batch_size, shuffle=True, drop_last=True)
    ...
    for ep in range(1, epoches+1):
        ...
        for batch, semi_batch in zip(loader, semi_loader):
            predictions, l1, _ = model.run_on_batch(semi_batch, semi_supervised=True)
            predictions, l2, _ = model.run_on_batch(semi_batch, semi_supervised=False)

            losses = {k: l1.get(k, 0) + l2.get(k, 0) for k in set(l1) | set(l2)} # merging two dictionaries
