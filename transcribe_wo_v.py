...
def transcribe(model, audio):
    ...
    mel = melspectrogram(audio.reshape(-1, audio.shape[-1])[:, :-1])
    mel = mel.transpose(-1, -2)
    onset_pred, frame_pred = model(mel)
...
def transcribe_file(model_file, flac_paths, save_path, sequence_length,
    ...
    for flac_path in flac_paths:
        ...
        p_est, i_est = extract_notes_wo_velocity(predictions['onset'], predictions['frame'], predictions['velocity'], onset_threshold, frame_threshold)
        ...
        save_midi(midi_path, p_est, i_est, 127)
