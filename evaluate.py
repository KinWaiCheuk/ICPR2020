...
def evaluate_wo_velocity(data, model, onset_threshold=0.5, frame_threshold=0.5, save_path=None):
    ...
    for label in data:
        ...
        # print(f"pred shape = {pred['frame'].shape}\t"
        #       f"file name = {label['path']}")
...
def evaluate_wo_velocity_onset(data, model, onset_threshold=0.5, frame_threshold=0.5, save_path=None):
    metrics = defaultdict(list)
    ...
    for label in data:
        ...
        p_ref, i_ref = extract_notes_wo_velocity(label['frame'], label['frame'])
        p_est, i_est = extract_notes_wo_velocity(pred['frame'], pred['frame'], onset_threshold, frame_threshold)
...
def evaluate_file(model_file, dataset, dataset_group, sequence_length, save_path,
    ...
    metrics = evaluate_wo_velocity(tqdm(dataset), model, onset_threshold, frame_threshold, save_path)
