...
def save_dict(obj, name ):
    with open('./runs_AE/config/'+ name + '.pkl', 'wb') as f:
        pickle.dump(obj, f, pickle.HIGHEST_PROTOCOL)

def load_obj(name):
    with open('./runs_AE/config/' + name + '.pkl', 'rb') as f:
        return pickle.load(f)

@ex.config
def config():
    logdir = 'runs_AE/only_transcription' + '-' + datetime.now().strftime('%y%m%d-%H%M%S')
