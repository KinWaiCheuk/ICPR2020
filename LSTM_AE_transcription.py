...
@ex.config
def config():
    logdir = f'runs_AE/LSTM_CQTbatch32_WNet_narrow_z_dsksize{ds_ksize}_dsstride_{ds_stride}_' + '-' + datetime.now().strftime('%y%m%d-%H%M%S')
...
def train(logdir, device, iterations, resume_iteration, checkpoint_interval, train_on, batch_size, sequence_length,
    ...
    if resume_iteration is None:
        model = LSTM_AE(ds_ksize,ds_stride)
        model.to(device)
