...
from evaluate import evaluate, evaluate_wo_velocity, evaluate_wo_velocity_onset # These two lines requires GPU
from onsets_and_frames import *
from onsets_and_frames.transcriber import *
...
def config():
    ...
    logdir = 'runs/framesLSTM_MAPSv1-' + ','.join(str(x) for x in TCN_layers) + '-' + datetime.now().strftime('%y%m%d-%H%M%S')
...
def train(logdir, device, iterations, resume_iteration, checkpoint_interval, train_on, batch_size, sequence_length,
    ...
    if resume_iteration is None:
        model = Frames_LSTM(N_MELS, MAX_MIDI - MIN_MIDI + 1, model_complexity).to(device)
    ...
    for ep in range(1, epoches):
        ...
        if (ep+1)%10 == 0 and ep+1 > 20:
            model.eval()
            with torch.no_grad():
                for key, values in evaluate_wo_velocity_onset(validation_dataset, model).items():
